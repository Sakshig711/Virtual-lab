...
const Quiz = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/api/get-all-quiz")
  ...
  const handleSave = async (assignmentId, questionId) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/get-all-quiz/${assignmentId}/questions/${questionId}`,
        {
          method: "PUT",
            ...
            assignment.assignmentId === assignmentId
              ? {
                  ...
                  questions: assignment.questions.map((q) =>
                    q.id === questionId ? { ...q, ...editedData } : q
                  ),
                }
              : assignment
          )
          ...
          <div
            ...
            onClick={() =>
              setSelectedAssignment(
                selectedAssignment === assignment.assignmentId ? null : assignment.assignmentId
              )
            }
          >
      ...
      {selectedAssignment !== null && (
        <div>
          ...
          <table className="questions-table">
            ...
            <tbody>
              {assignments
                .find((a) => a.assignmentId === selectedAssignment)
                ?.questions.map((q, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      {editingQuestion === q.id ? (
                        <input
                          ...
                          onChange={(e) =>
                            setEditedData({ ...editedData, correctOption: e.target.value })
                          }
                        />
                      ) : (
                    ...
                    <td>
                      {editingQuestion === q.id ? (
                        <button className="save-button" onClick={() => handleSave(selectedAssignment, q.id)}>
                          Save
                        </button>
                      ) : (
                        <button className="edit-button" onClick={() => handleEditClick(selectedAssignment, q)}>
                          Edit
                        </button>
                      )}
                    </td>
